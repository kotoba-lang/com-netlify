(ns netlify.main
  "Kotodama WASM entrypoint for the Netlify clean-room actor (L4) — Clojure port.
  ...
  facts (`netlify.<Entity>/<field>`); `*store*` is the in-memory materialization
  used by the contract test and by the WASM runtime before a live engine binds."
  (:require [clojure.string :as str]))

(def ns-prefix "netlify")
(def tier "L4")
...
(def entity-specs
  [{:entity "Site"    :plural "sites"    :id-prefix "netlify_sit"
    :fields [:name :url :sslUrl :adminUrl :state :customDomain :accountId :ssl :forceSsl] :required [:name]
    :coerce {:ssl :bool :forceSsl :bool}           :refs {}}
   {:entity "Deploy"  :plural "deploys"  :id-prefix "netlify_dep"
    :fields [:siteId :url :state :draft :branch :commitRef :locked] :required [:url]
    :coerce {:draft :bool :locked :bool}           :refs {:siteId "Site"}}
   {:entity "Build"   :plural "builds"   :id-prefix "netlify_bui"
    :fields [:deployId :sha :done :error]          :required [:sha]
    :coerce {:done :bool}                          :refs {:deployId "Deploy"}}
   {:entity "DnsZone" :plural "dnszones" :id-prefix "netlify_dns"
    :fields [:siteId :domain :dedicated :ipv6Enabled :accountId] :required [:domain]
    :coerce {:dedicated :bool :ipv6Enabled :bool}  :refs {:siteId "Site"}}
   {:entity "Hook"    :plural "hooks"    :id-prefix "netlify_hoo"
    :fields [:siteId :type :event :disabled]       :required [:type]
    :coerce {:disabled :bool}                      :refs {:siteId "Site"}}
   {:entity "Form"    :plural "forms"    :id-prefix "netlify_for"
    :fields [:siteId :name :submissionCount]       :required [:name]
    :coerce {:submissionCount :int}                :refs {:siteId "Site"}}])
...
(defn emit-facts
  "EAVT facts for one record: {\"netlify.<Entity>/<field>\" v ...}. The datomic
...
(defn healthz [] [{:status "ok" :actor "netlify-compat" :tier tier :entities entities} 200])
...
(defn start! [] :netlify-compat/ready)
